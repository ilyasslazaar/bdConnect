...
export type IExecutionState = IPaginationBaseState;
...
export class Execution extends React.Component<IExecutionProps, IExecutionState> {
  ...
  render() {
    const { executionList, match, totalItems } = this.props;
    return (
      <div>
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={this.sort('exDate')}>
                  Ex Date <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={this.sort('status')}>
                  Status <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Query <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {executionList.map((execution, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={execution.exDate} format={APP_LOCAL_DATE_FORMAT} />
                  </td>
                  <td>{execution.status ? 'true' : 'false'}</td>
                  <td>{execution.query ? <Link to={`query/${execution.query.id}`}>{execution.query.id}</Link> : ''}</td>
